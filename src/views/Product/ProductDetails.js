...
class ProductDetails extends Component {
  ...
  getImageGallery() {
    return this.props.product.images.map(image => ({ original: image }));
  }

  getPermalLink() {
      return 'http://35.187.246.124:8081/catalog/product/view/id/'+ this.props.product.id;
  }
    ...
    return (
      <div>
        ...
        <Card centered>
          ...
          <Card.Content>{this.props.product.is_salable ? 'In Stock' : 'Out of Stock'}</Card.Content>
          <Card.Content>
        ...
        <SocialBox permalink={this.getPermalLink()} />
      </div>
    );
  }
}
